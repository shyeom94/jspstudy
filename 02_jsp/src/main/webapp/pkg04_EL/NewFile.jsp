...
<%--
  ...
  2. Bind 영역(데이터 저장 영역)에 저장된 값을 나타낼 수 있다. 
...
<%-- 
  Bind 영역 (데이터 저장 영역) 
  1. pageContext : this(NewFile_jsp)  : 현재 JSP 안 NewFile.jsp 에서만 접근 가능하다. 
  2. request     : HttpServletRequest : 응답 전까지 접근 가능하다. 
  3. session     : HttpSession        : 브라우저 닫기 전까지 접근 가능하다. 
  4. application : ServletContext     : 애플리케이션 종료(서비스 중지) 전까지 접근 가능하다. 
--%>
...
<body>
  ...
  %>
  <div>${book.title}</div>
  <%-- book.getTitle() 방식으로 호출된다. --%>
  <div>${book.author}</div>
  <%-- book.getAuthor() 방식으로 호출된다. --%>
  <div>${book.price}</div>
  <%-- book.getPrice() 방식으로 호출된다. --%>
